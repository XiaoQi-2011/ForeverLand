...
struct EData {
    int itemID = 0;
    vector<int> values;
    explicit EData(int itemID, vector<int> values) {
        this->itemID = itemID;
...
EData getEquipmentData(int itemID) {
...
inline void initEquipmentItem() {
    ...
    for (EData data : EquipmentData) {
        int id = data.itemID;
        vector<int> values = data.values;
        stringstream ss;
        ...
        if (init_item[id].quality == 7) {
            int colour = init_item[id].back_ground < 50 ? init_item[id].back_ground + 50 : init_item[id].back_ground - 70;
            string color = "\033[" + to_string(colour) + "m";
...
void takeOffEquipment(Player& player, int slot_id) {
    if (player.getData(slot_id) != -1) {
        int item_id = player.getData(slot_id);
        vector<int> values = getEquipmentData(item_id).values;
        for (int i = 0; i < values.size(); i++) {
...
void putOnEquipment(Player& player, int item_id, int slot_id) {
    if (player.getData(slot_id) != -1) {
