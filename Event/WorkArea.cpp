...
bin:
    ...
    printf("-背包-\n\n");
    std::vector<std::pair<std::string, int>> itemList = player.getItemList({"all"}, false);
...
decompose:
    system("cls");
    printf("-分解区-\n\n");
    std::vector<std::pair<std::string, int>> itemList = player.getItemList({"equipment"}, false);
    for (int i = 0; i < itemList.size(); i++)
    {
        printf("%d.%s ", i + 1, itemList[i].first.c_str());
        int id = itemList[i].second;
        std::string type = init_item[id].type;
        if (type.starts_with("equipment_"))
        {
    ...
    std::string type = init_item[itemId].type;
    if (type.starts_with("equipment_"))
    {
