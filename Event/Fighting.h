...
void initKeys(Player& player) {
    ...
    if (keys.empty()) {
        player.strData[2]->value = init_strData[2].value;
        return;
    }
    attackKey = keys[0][0];
    potionKey = keys[1][0];
...
int judgmentWin() {
    ...
    if (enemyHealth <= 0) {
        ...
        vector<pair<string, int>> items = fEntity->capture.getItemList(vector<string>{"all"}, false);
...
void entityAttack() {
    while (true) {
        ...
        if (shanbi <= fPlayer->getData(10)) {
            ...
            printf("[%s]攻击你造成\033[1;33m%d\033[0m伤害，\033[1;34m你的血量:\033[1;31m%d/%d ", name.c_str(), damage, meHealth, fPlayer->getData(7));
            ...
            if (xixue <= fEntity->getData(11)) {
                ...
                printf("\033[1;31m[-][%s]吸取你\033[0;31m%d\033[1;31m血量，对方血量:\033[1;31m%d/%d\033[0m ",name.c_str(), xixue, enemyHealth, fEntity->getData(7));
...
inline bool fight(Player& player, Entity& entity, Player& real_player, bool isContinue = false) {
    initKeys(player);
    win = 0;
    times = 0;
    ...
    thread(entityAttack).detach();
    thread(MainLoop).detach();
    while (true) {
        if (win != 0) {
            string str;
            cout << "输入任意内容继续..." << endl;
        ...
        if (key == attackKey) {
            ...
            if (shanbi <= entity.getData(10)) {
                ...
                printf("\033[1;34m>\033[0m你攻击了[%s]造成\033[1;33m%d\033[0m伤害，对方血量:\033[1;31m%d/%d\033[0m ", name.c_str(), damage, enemyHealth, entity.getData(7));
            ...
            switch (item_id) {
                case 57: {
                    if (healthCD == -1) {
                        ...
                        meHealth += health;
                        meHealth = meHealth > player.getData(7) ? player.getData(7) : meHealth;
                        printf("\033[1;31m[+]你使用了小型治疗药水 +%d生命值，\033[1;34m你的血量:\033[1;31m%d/%d ",
                               health, meHealth, fPlayer->getData(7));
                        int healthP = meHealth * 100 / fPlayer->getData(7);
                        cout << healthColors[healthP / 20] << healthP << "%\033[0m" << endl;
                    } else printf("药水冷却中，剩余%d秒\n", healthCD);
                    break;
                }
                case 58: {
                    if (healthCD == -1) {
                        ...
                        meHealth += health;
                        meHealth = meHealth > player.getData(7) ? player.getData(7) : meHealth;
                        printf("\033[1;31m[+]你使用了大型治疗药水 +%d生命值，\033[1;34m你的血量:\033[1;31m%d/%d ",
                               health, meHealth, fPlayer->getData(7));
                        int healthP = meHealth * 100 / fPlayer->getData(7);
                        cout << healthColors[healthP / 20] << healthP << "%\033[0m" << endl;
                    } else printf("药水冷却中，剩余%d秒\n", healthCD);
                ...
                case 59: {
                    if (strengthCD == -1) {
                        ...
                        printf("\033[1;33m[+]你使用了小型力量药水 +%d%%伤害加成\033[0m\n", strength);
                    } else printf("药水冷却中，剩余%d秒\n", strengthCD);
                    break;
                }
                case 60: {
                    if (strengthCD == -1) {
                        ...
                        player.data[8]->value += strength;
                        printf("\033[1;33m[+]你使用了大型力量药水 +%d%%伤害加成\033[0m\n", strength);
                    } else printf("药水冷却中，剩余%d秒\n", strengthCD);
                    break;
                }
                case 61: {
                    if (resistanceCD == -1) {
                        ...
                        printf("\033[1;30m[+]你使用了小型抗性药水 +%d%%伤害减免\033[0m\n", resistance);
                    } else printf("药水冷却中，剩余%d秒\n", resistanceCD);
                    break;
                }
                case 62: {
                    if (resistanceCD == -1) {
                        ...
                        player.data[9]->value += resistance;
                        printf("\033[1;30m[+]你使用了大型抗性药水 +%d%%伤害减免\033[0m\n", resistance);
                    } else printf("药水冷却中，剩余%d秒\n", resistanceCD);
                    break;
                }
            }
...
inline void createFightArea(Player& player, const string& str) {
    system("cls");
    printf("%s\n", str.c_str());
    initKeys(player);
