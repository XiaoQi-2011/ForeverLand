...
showPlayerItems:
    ...
    for (int i = 0; i < DATA_NUM; i++)
    {
        if (getInitData()[i].type == "slot")
        {
            ...
            printf("a%d.%s: %s\n", i - 16, getInitData()[i].name.c_str(), item_name.c_str());
        }
    }
    printf("\n[物品]\n");
    std::vector<std::pair<std::string, int>> items = player.getItemList({"all"}, true);
    int num = items.size();
    for (int i = 0; i < num; i++)
    {
    ...
    if (choice == -3)
    {
        ...
        if (player.getData(slot_id) != -1)
        {
            ...
            std::cout << "已卸下装备" << std::endl;
            system("pause");
        }
        else
        {
            std::cout << "该槽位为空" << std::endl;
            system("pause");
        }
    ...
    std::string item_type = init_item[item_id].type;

    int count = 1;
    bool useFlag = true;
    for (const std::string& type : onlyOnceTypes)
    {
        if (item_type.starts_with(type))
        {
            useFlag = false;
            break;
        }
    ...
    if (useFlag)
    {
        std::cout << "请输入使用数量(默认为1，all为全部):" << std::endl;
        std::string input2;
    ...
    for (int cnt = 1; cnt <= count; cnt++)
    {
        ...
        // 装备
        if (item_type.starts_with("equipment_"))
        {
            int slot_id = 0;
            ...
            if (item_type == "equipment_accessory")
            {
                ...
                if (isOn)
                {
                    std::cout << "每种饰品只能装备1个！" << std::endl;
                    system("pause");
                    goto showPlayerItems;
                }
                if (!flag)
                {
                    std::cout << "饰品槽位已满" << std::endl;
                    system("pause");
                    goto showPlayerItems;
                }
            }
        ...
        if (item_type == "chest")
        {
            ...
            player.items[item_id]->count--;
            std::cout << "使用成功！你获得了：" << std::endl;
            std::cout << init_item[id].name << " ×1" << std::endl;
            success = true;
        }

        // 福袋
        if (item_type.starts_with("fudai_"))
        {
        ...
        // 刷新券
        if (item_type.starts_with("shuaxin"))
        {
            if (item_id == 98)
            {
                reflashZahuo(player);
            ...
            {
                printf("请输入世界Boss序号: \n");
                int choice1 = getChoice(1, bossCount);
                if (choice1 == -1 || choice1 == -2) goto showPlayerItems;
                worldBosses[choice1 - 1].isFlashed = true;
            }
            printf("使用成功！\n");
            player.items[item_id]->count--;
            success = true;
        ...
        // 消耗品
        if (item_type.starts_with("consumables"))
        {
            // 补充体力
            if (item_id == 101)
            {
    ...
    if (!success) std::cout << "该物品无法使用或使用失败" << std::endl;
    system("pause");
    goto showPlayerItems;
}
