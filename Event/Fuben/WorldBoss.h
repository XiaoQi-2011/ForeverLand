...
struct WorldBoss
{
    ...
    std::vector<BossCapture> capture;

    WorldBoss() = default;

    WorldBoss(std::string name, std::string introduce, std::string color, int level, int flashTime, int jb_need, int zs_need,
              std::string special,
              const VP& data, std::vector<BossCapture> capture, const VP& item_need = {})
    {
        if (!color.empty())
        {
...
inline void lotteryCapture(Entity& boss, const std::vector<BossCapture>& capture)
{
    for (auto i : capture)
    {
...
{
    worldBosses[0] = WorldBoss("白王·伊邪那美", "仅次于黑王的最强龙王之一", "\033[1;37m",
              100, 900,
              100, 0, "",
              {{1, 5000}, {4, 470}, {5, 128}, {6, 10000}, {7, 10000}, {14, 43}},
              {{86, 1, 1, 40}, {94, 4, 5, 100}, {96, 2, 3, 100}}
    );
    worldBosses[1] = WorldBoss("诸神之父·奥丁", "北欧神话中的众神之王，诸神之父，手持掌握命运的永恒之枪「昆古尼尔」，在他面前你将无法闪避", "\033[1;32m",
              103, 900,
              100, 0, "你无法在此Boss前闪避",
              {{1, 5000}, {4, 440}, {5, 150}, {6, 12000}, {7, 12000}},
              {{87, 1, 1, 40}, {94, 4, 5, 100}, {96, 2, 3, 100}}
    );
    worldBosses[2] = WorldBoss("Herobrine", "\"Tell me, Do gods bleed?\"\n\"No, but you will\"", "\033[0;36m",
              92, 600,
              100, 0, "",
              {{1, 3000}, {4, 405}, {5, 135}, {6, 7000}, {7, 7000}},
              {{91, 1, 1, 90}, {94, 2, 3, 100}, {96, 1, 2, 100}}
    );
    worldBosses[3] = WorldBoss("深渊领主", "来自深渊的目光正在凝视着你", "\033[0m",
              90, 600,
              100, 0, "",
              {{1, 3000}, {4, 400}, {5, 130}, {6, 7000}, {7, 7000}},
              {{90, 1, 1, 80}, {94, 2, 3, 100}, {96, 1, 2, 100}}
    );
    worldBosses[4] = WorldBoss("神秘人", "???", "\033[1;35m",
              95, 0,
              400, 40, "",
              {{1, 5000}, {6, 9000}, {7, 9000}},
              {{8, 1, 2, 100}, {52, 1, 2, 100}}
    );
    worldBosses[5] = WorldBoss("神官", "只有通过了我的考核，你才能飞升成神", "\033[1;34m",
              120, 0,
              500, 50, "",
...
worldBoss:
    ...
    for (int i = 1; i <= bossCount; i++)
    {
        ...
        if (boss.flashTime != 0)
        {
            ...
            std::string shuaxin = boss.isFlashed ? "\033[0;32m已刷新" : "\033[0;31m未刷新";
        ...
        if (boss.needItem)
        {
            ...
            std::vector<std::pair<std::string, int>> itemList = boss.item_need.getItemList({"all"}, false);
        ...
        for (const auto& capture : boss.capture)
        {
            ...
            {
                printf("%s: %d~%d (%d%%)\n", init_item[capture.id].name.c_str(), capture.minCount, capture.maxCount,
                       capture.percent);
            }
        }
        //printf("\n");
