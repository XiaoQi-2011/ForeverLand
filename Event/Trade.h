...
void initShenmi(Player& player) {
    string str = player.getStrData(1);
    if (str == "empty") {
        reflashShenmi(player);
        return;
    }
    vector<string> IDs = split(str, '$');
...
void initZahuo(Player& player) {
    string str = player.getStrData(0);
    if (str == "empty") {
        reflashZahuo(player);
        return;
    }
...
const string sellColor = "\033[1;34m";
const string buyColor = "\033[1;32m";
inline TradeChoice getTradeChoice() {
    string input;
    getline(cin, input);
    if (input.empty()) return TradeChoice(-1);
    if (input == "back") return TradeChoice(-2);
    vector<string> choices = split(input, ' ');
    TradeChoice choice;
...
inline bool trade(Player& player, int itemId, int count, bool isBuy, float override = 1.0) {
...
inline int getAllCount(Player& player, int itemId, bool isBuy, float override = 1.0) {
    if (isBuy) {
        ...
        int count = min(jb_count, zs_count);
        return count;
    } else {
...
inline void TradeMenu(Player& player) {
    Trade:
    system("cls");
    printf("--交易区--\n");
    ...
    printf("> ???\n");
    for (int i = 0; i < 6; i++) {
        int jb_value = init_item[shenmiProduct[i]].JB_value * 1.4, zs_value = init_item[shenmiProduct[i]].ZS_value * 1.1;
